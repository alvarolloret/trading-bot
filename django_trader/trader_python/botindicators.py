...
class BotIndicators(object):
    ...
    def EMA(self, prices, period, currentPrice):
        if (len(prices) > 1):
            ...
            weights = None
            # if period=10: array([ 0.36787944,  0.41111229,  0.45942582,  0.51341712,  0.57375342, 0.64118039,  0.71653131,  0.8007374 ,  0.89483932,  1. ])
            weights = numpy.exp(numpy.linspace(-1., 0., period))
            # if period=10: array([ 0.03857234,  0.04142829,  0.0444957 ,  0.04779023,  0.05132868, 0.05512913,  0.05921097,  0.06359504,  0.0683037 ,  0.07336101, 0.07879276,  0.08462669,  0.09089257,  0.09762239,  0.10485049])
            weights /= weights.sum()

            a = numpy.convolve(dataPrices, weights, mode='full')[
                               :len(dataPrices)]
            return a[-1]
        else:
            return currentPrice
